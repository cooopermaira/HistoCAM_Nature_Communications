...
namespace pathCam {
  ...
  std::string printSubKeys(LayeredConfiguration::Ptr config, std::string short_key, std::string full_key,
  ...
  bool BatchCam::parseConfig(LayeredConfiguration::Ptr pConf) {
    ...
    if (pConf->has("io")) {
      ...
      if (pConf->has("io.inferencing.classify.classifier")) {
        classifying = true;
        ...
        while (pConf->has("io.inferencing.classify.classes.class[" + std::to_string(index) + "].id")) {
          ...
          std::string basePath = "io.inferencing.classify.classes.class[" + std::to_string(index) + "]";

          classInfo.id = pConf->getInt(basePath + ".id");
          ...
          classesInfo.push_back(classInfo);
          index++;
        }
      }
      else {
      ...
      if (pConf->has("io.flat_field_images")) {
        if (pConf->has("io.flat_field_images.twoX")) {
          ...
          if (flat_field_file_2x.getExtension() != "png" && flat_field_file_2x.getExtension() != "tif" &&
              flat_field_file_2x.getExtension() != "tiff" && flat_field_file_2x.getExtension() != "Raw") {
        ...
        if (pConf->has("io.flat_field_images.fourX")) {
          ...
          if (flat_field_file_4x.getExtension() != "png" && flat_field_file_4x.getExtension() != "tiff" &&
              flat_field_file_4x.getExtension() != "Raw") {
        ...
        if (pConf->has("io.flat_field_images.tenX")) {
          ...
          if (flat_field_file_10x.getExtension() != "png" && flat_field_file_10x.getExtension() != "tiff" &&
              flat_field_file_10x.getExtension() != "Raw") {
            logger->warning("Only PNG, Raw, or TIF outputs supported. No image output.");
        ...
        if (pConf->has("io.flat_field_images.twentyX")) {
          ...
          if (flat_field_file_20x.getExtension() != "png" && flat_field_file_20x.getExtension() != "tiff" &&
              flat_field_file_20x.getExtension() != "Raw") {
            logger->warning("Only PNG, Raw, or TIF outputs supported. No image output.");
    ...
    if (pConf->has("registration")) {
      if (pConf->has("registration.detector")) {
        if (pConf->has("registration.detector.features")) {
          ...
          if (pConf->has("registration.detector.features.params")) {
            switch (feature_type) {
              case _SIFT:
                logger->warning("Params for this feature not yet implemented");
                return false;
                break;
              case _SURF:
                logger->warning("Params for this feature not yet implemented");
              ...
              case _ORB:
                ...
                float scaleFactor;
                try {
                  scaleFactor = pConf->getDouble(
                      "registration.detector.features.params[@scaleFactor]");
                } catch (std::string bad_input) {
                ...
                int edgeThreshold;
                try {
                  edgeThreshold = pConf->getInt(
                      "registration.detector.features.params[@edgeThreshold]");
                } catch (std::string bad_input) {
                ...
                std::string temp = pConf->
                    getString("registration.detector.features.params[@scoreType]");
                ...
                int fastThreshold;
                try {
                  fastThreshold = pConf->getInt(
                      "registration.detector.features.params[@fastThreshold]");
                } catch (std::string bad_input) {
      ...
      if (pConf->has("registration.image")) {
        ...
        if (pConf->has("registration.image.scope_radius")) {
          ...
          try {
            scope_radius = pConf->getUInt("registration.image.scope_radius") - 50;
          } catch (std::string bad_input) {
  ...
  public:
    ...
    bool registration(unsigned int thread_id) {
      ...
      for (unsigned int i = start; i < end; i++) {
        ...
        last_registered->create_reg_image(parent->scale_factor, parent->crop_factor, parent->debayer,
                                          parent->interpolation, parent->real);
        next_image->create_reg_image(parent->scale_factor, parent->crop_factor, parent->debayer,
  ...
  bool BatchCam::run() {
    ...
    for (unsigned int i = 0; i < threads; i++) {
      if (!runnable[i].successful) {
        logger->error("Registration failed. %d", i);
        return false;
      }
    }
    ...
    if (out_image.toString() != "") {
      ...
      good = compositing();
      if (!good) {
        logger->error("Compositing failed.");
        return false;
      }
  ...
  void BatchCam::find_overlaps() {
    ...
    for (unsigned int i = 0; i < overlapM.overlap.size(); i++) {
      for (unsigned int j = 0; j < overlapM.overlap[i].size(); j++) {
        if (matchM.match[i][j] != NULL) {
          overlapM.overlap[i][j] = 1.0;
          continue;
        }
        //area
    ...
    for (int i = img.rows / 2 - radius; i < img.rows / 2 + radius; i++) {
      ...
      for (int j = img.cols / 2 - radius; j < img.cols / 2 + radius; j++) {
        ...
        if (dist_from_center <= peak * radius) {
          img.at<int>(i, j) = std::max(0, int(floor(
              (255 - center_val) / (peak * radius) * dist_from_center +
              center_val)));
        } else {
          img.at<int>(i, j) = std::max(
              0, int(floor(-255 / ((1.0 - peak) * radius) * (dist_from_center - radius))));
        }
      }
    }
  ...
  Mat dome_score_image(int rows, int cols, int radius) {
    ...
    for (int i = 0; i < img.rows; i++) {
      ...
      for (int j = 0; j < img.cols; j++) {
        jdist = pow(j - img.cols / 2, 2);
        img.at<uint8_t>(i, j) = std::max((uint8_t) 0,
                                         uint8_t(floor(255.0 / rad_sq * (rad_sq - idist - jdist))));
      }
    }
    return img;
  }
  ...
  bool BatchCam::compositing() {
    Mat3b combined(combined_box.max_y - combined_box.min_y, combined_box.max_x - combined_box.min_x,
                   Vec3b(0, 0, 0));

    Mat3f flat_field_buffer(combined_box.max_y - combined_box.min_y, combined_box.max_x - combined_box.min_x,
                            Vec3f(0, 0, 0));
    ...
    for (unsigned int i = 0; i < images.size(); i++) {
      if (reg_results[i]->successful) {
        k++;
        Mat use_locations = mask.mul(
            quality_score > combined_z_buffer(Rect(box[i].min_x, box[i].min_y, images[i]->width,
                                                   images[i]->height)));
        ...
        */
        ...
        if (temp->label == Image::_2X && images[i]->height == flat_field.rows && images[i]->width == flat_field.
            cols) {
          flat_field.copyTo(
              flat_field_buffer(Rect(box[i].min_x, box[i].min_y, flat_field.cols, flat_field.rows)),
              use_locations);
        }
        ...
        quality_score.copyTo(
            combined_z_buffer(Rect(box[i].min_x, box[i].min_y, images[i]->width, images[i]->height)),
            use_locations);
      }
    }
