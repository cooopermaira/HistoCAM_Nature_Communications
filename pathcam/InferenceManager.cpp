...
//         }
  ...
  void InferenceManager::run() {
    ...
    torch::Tensor tileEmbeds = torch::empty({0, embedSize},
    ...
    if (parent->aggregating) {
      ...
      torch::Tensor coordsTensor = torch::empty({long(tileCoordToTensorIndex.size()), 2},
      ...
      //debug
      tileEmbeds = torch::cat({tileEmbeds.index({torch::indexing::Slice(), torch::indexing::Slice(0, 768)}), coordsTensor}, 1);
