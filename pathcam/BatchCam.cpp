...
//      }
        ...
        next_image->create_reg_image(parent->scale_factor, parent->crop_factor, parent->debayer, parent->interpolation,
                                     parent->real);
      ...
      for (int j = img.cols / 2 - radius; j < img.cols / 2 + radius; j++) {
        ...
        if (dist_from_center <= peak * radius) {
          img.at<int>(i, j) = std::max(0, int(floor(
              (255 - center_val) / (peak * radius) * dist_from_center + center_val)));
        } else {
    ...
    for (unsigned int i = 0; i < images.size(); i++) {
      if (reg_results[i]->successful) {
        ...
        Mat use_locations = mask.mul(
            quality_score > combined_z_buffer(Rect(box[i].min_x, box[i].min_y, images[i]->width, images[i]->height)));
        ...
        */
        ...
        if (temp->label == Image::_2X && images[i]->height == flat_field.rows && images[i]->width == flat_field.cols) {
          flat_field.copyTo(flat_field_buffer(Rect(box[i].min_x, box[i].min_y, flat_field.cols, flat_field.rows)),
                            use_locations);
        }
        ...
        quality_score.copyTo(combined_z_buffer(Rect(box[i].min_x, box[i].min_y, images[i]->width, images[i]->height)),
                             use_locations);
